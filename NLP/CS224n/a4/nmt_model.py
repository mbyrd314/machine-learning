...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.h_projection = nn.Linear(2*hidden_size, hidden_size, bias=False) # These are linear transforms,
        self.c_projection = nn.Linear(2*hidden_size, hidden_size, bias=False) # not affine
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for t in range(len(target_padded)):
            Y_t = Y[t].squeeze(dim=0)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.squeeze(enc_hiddens_proj.bmm(torch.unsqueeze(dec_hidden,2)),2)
        ...
        a_t = torch.squeeze(torch.unsqueeze(alpha_t, 1).bmm(enc_hiddens), 1)
